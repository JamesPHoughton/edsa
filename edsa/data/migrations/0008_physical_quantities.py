...
from edsa.data.models import PhysicalQuantity

class Migration(DataMigration):

    quantity_names = [
        #   SI base unit types
        'length', 'mass', 'time', 'current', 'temperature', 'luminance', 'amount',
        #   Other: General
        'frequency', 'angle', 'solid angle', 'power', 'energy', 'density', 'velocity', 'acceleration',
        #   Other: Electrical quantities
        'voltage', 'resistance', 'capacitance', 'inductance', 'impedance', 'admittance', 'conductance', 'charge',
        #   Other: Magnetic quantities
        'magnetic flux', 'magnetic field', 'magnetic flux density',
        #   Other: Mechanical quantities
        'force', 'pressure', 'viscosity', 
        ]

    def forwards(self, orm):
        for qn in Migration.quantity_names:
            pq, created = PhysicalQuantity.objects.get_or_create(label=qn)

    def backwards(self, orm):
        #   I guess we don't want to delete these things, because
        #   a lot of other objects depend on them.
        #   This would be done by:
        #   PhysicalQuantity.objects.filter(label__in=Migration.quantity_names).delete()
