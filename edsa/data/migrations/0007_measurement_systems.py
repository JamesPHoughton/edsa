...
from edsa.data.models import MeasurementSystem

class Migration(DataMigration):

    system_names = {1: 'SI', 2: 'Imperial'}

    def forwards(self, orm):
        for sn in Migration.system_names:
            ms, created = MeasurementSystem.objects.get_or_create(id=sn, name=Migration.system_names[sn])

    def backwards(self, orm):
        #   I guess we don't want to delete these things, because
        #   a lot of other objects depend on them.
        #   This would be done by:
        #   MeasurementSystem.objects.filter(name__in=Migration.system_name).delete()
