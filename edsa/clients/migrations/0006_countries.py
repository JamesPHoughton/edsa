...
from edsa.clients.models import Country

class Migration(DataMigration):

    def forwards(self, orm):
        "Write your forwards methods here."
        file = open('../data/countries_iso3166.txt')
        data = file.readlines()
        for line in data:
            line_elements = line.split(';')
            name = line_elements[0].strip()
            code = line_elements[1].strip()
            country, created = Country.objects.get_or_create(iso_code = code, name = name.lower().title())
            print 'Added country: %s, code: %s' % (name.lower().title(), code)
        file.close()

    def backwards(self, orm):
        "Write your backwards methods here."
        #   Do nothing - who cares if the countries are left alone?
        pass
