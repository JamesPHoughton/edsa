...
from edsa.clients.models import RegisteredPythonModule
from edsa.data.models import VarType

class Migration(SchemaMigration):

    def forwards(self, orm):
        
        # Renaming field 'VarType.field'
        db.rename_column('data_vartype', 'field_id', 'field_class_id')

        # Deleting field 'VarType.base_field'
        db.delete_column('data_vartype', 'base_field')

        # Ensure that we have the basic Django form fields and VarTypes registered
        vt_dict = { 'Integer': 'django.forms.IntegerField',
                    'Float':   'django.forms.FloatField',
                    'String':  'django.forms.CharField',
                    'Boolean': 'django.forms.BooleanField',
                    'Array':   'edsa.data.fields.ArrayField',
                    'File':    'django.forms.FileField'
                   }
        for key in vt_dict:
            rpm = RegisteredPythonModule.register(vt_dict[key])
            vt, created = VarType.objects.get_or_create(label=key)
            vt.field_class = rpm
            vt.save()

    def backwards(self, orm):
        
        # Renaming field 'VarType.field_class'
        db.rename_column('data_vartype', 'field_class_id', 'field_id')

        # Adding field 'VarType.base_field'
        db.add_column('data_vartype', 'base_field', self.gf('django.db.models.fields.CharField')(max_length=20, null=True, blank=True), keep_default=False)
    ...
    models = {
        ...
        'data.vartype': {
            ...
            'field_class': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['clients.RegisteredPythonModule']", 'null': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
