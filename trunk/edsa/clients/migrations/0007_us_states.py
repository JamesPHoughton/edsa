...
from edsa.clients.models import StateProvince, Country

class Migration(DataMigration):

    def forwards(self, orm):
        "Write your forwards methods here."
        file = open('../data/us_states.txt')
        data = file.readlines()
        for line in data:
            line_elements = line.split('\t')
            name = line_elements[1].strip()
            code = line_elements[0].strip()
            target_country, created = Country.objects.get_or_create(iso_code='US')
            sp, created = StateProvince.objects.get_or_create(country=target_country, iso_code=code, name=name)
            print 'Added state: %s, code: %s' % (name, code)
        file.close()
